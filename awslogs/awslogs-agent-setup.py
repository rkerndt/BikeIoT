...
class CloudWatchLogsAgentSetup:
    Rhel, Ubuntu, AmazonLinux, CentOS, Raspbian, Debian, Arch = range(7)
    ...
    def install(self, package):
        ...
        if self.os_flavor == self.Arch:
            subprocess.call([installer, '-S', package], stderr=self.log_file, stdout=self.log_file)
        else:
...
def main():
    ...
    if python_version < (2,6) or python_version >= (3,7):
        fail("This script only supports python version 2.6 - 3.6", PLATFORM_NOT_SUPPORTED)
